export async function getWog(id: number | string) {
  ...
  try {
    const response = await fetch(new URL(`https://www.worldofgothic.de/dl/download_${id}.htm`), {
      method: 'GET',
      headers: {
        Accept: 'text/html',
      },
    ...
    if (match === null || match.length < 2) console.warn('Could not retrieve download count from WoG download page. Returning 0 downloads')
    else downloads = +match.slice(1)[0]
  } catch (error) {
